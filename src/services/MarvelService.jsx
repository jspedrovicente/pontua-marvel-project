...
class MarvelService {
    ...
    static async getAllCharacters() {
        ...
        try {
            const response = await axios.get(`${baseUrl}/v1/public/characters?&ts=${ts}&apikey=${process.env.REACT_APP_MARVEL_PUBLIC_API_KEY}&hash=${hash}`);
    ...
    static async searchCharacterByName(name) {
        ...
        const baseUrl = 'https://gateway.marvel.com:443'
        try {
            const response = await axios.get(`${baseUrl}/v1/public/characters?nameStartsWith=${name}&limit=100&ts=${ts}&apikey=${process.env.REACT_APP_MARVEL_PUBLIC_API_KEY}&hash=${hash}`);
            this.result = response.data.data.results;
            return this.result
        }            
        catch (error) {
